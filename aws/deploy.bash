...
if [ -z $VPC_PUBLIC_SUBNET_ID ]; then
	...
	aws ec2 modify-subnet-attribute --subnet-id $VPC_PUBLIC_SUBNET_ID --map-public-ip-on-launch
fi
...
if [ -z $SG_APP_ID ]; then
	aws ec2 create-security-group --vpc-id $VPC_ID --description $APP_SG_NAME --group-name $APP_SG_DESC > $TMP
...
if [ -z $SG_TUNERS_ID ]; then
	aws ec2 create-security-group --vpc-id $VPC_ID --description $TUNERS_SG_NAME --group-name $TUNERS_SG_DESC > $TMP
...
if [ -z $SG_DB_ID ]; then
	aws ec2 create-security-group --vpc-id $VPC_ID --description $DB_SG_NAME --group-name $DB_SG_DESC > $TMP
...
if [ -z $DB_INST_ID ] || [ -z $DB_INST_IP ]; then
	aws ec2 run-instances --image-id $EC2_AMI --count 1 --instance-type $EC2_INST_TYPE --key-name $EC2_KEYPAIR --security-group-ids $SG_DB_ID --subnet-id $VPC_PRIVATE_SUBNET_ID --user-data file://$DB_INST_USERDATA > $TMP 
	DB_INST_ID=$(cat $TMP | jq -r '.Instances[0].InstanceId')
	DB_INST_IP=$(cat $TMP | jq -r '.Instances[0].PrivateIpAddress')

	echo DB_INST_ID=$DB_INST_ID >> $PROGRESS
	echo DB_INST_IP=$DB_INST_IP >> $PROGRESS
fi

if [ -z $TUNERS_INST_ID ] || [ -z $TUNERS_INST_IP ]; then
	cat $TUNERS_INST_USERDATA_TEMPLATE | sed "s/CCR_MONGO_HOST/$DB_INST_IP/g" > $TUNERS_INST_USERDATA
	aws ec2 run-instances --image-id $EC2_AMI --count 1 --instance-type $EC2_INST_TYPE --key-name $EC2_KEYPAIR --security-group-ids $SG_TUNERS_ID --subnet-id $VPC_PUBLIC_SUBNET_ID --user-data file://$TUNERS_INST_USERDATA > $TMP 
...
if [ -z $APP_INST_ID ] || [ -z $APP_INST_IP ]; then
	cat $APP_ISNT_USERDATA_TEMPLATE | sed "s/CCR_MONGO_HOST/$DB_INST_IP/g" > $APP_ISNT_USERDATA
	aws ec2 run-instances --image-id $EC2_AMI --count 1 --instance-type $EC2_INST_TYPE --key-name $EC2_KEYPAIR --security-group-ids $SG_APP_ID --subnet-id $VPC_PUBLIC_SUBNET_ID --user-data file://$APP_ISNT_USERDATA > $TMP 
